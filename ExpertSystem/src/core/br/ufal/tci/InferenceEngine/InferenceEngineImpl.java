 ...
 * @author mfp
 * 
 *         Classe que implementa InferenceEngine
 * 
 */
public class InferenceEngineImpl implements InferenceEngine {
	...
	public InferenceEngineImpl(Interpreter ip, KnowledgeBase klb) {
		this.interpreter = ip;
		this.knowledgebase = klb;
		this.listOfVariables = new LinkedList<Variable>();
		this.setMethodInference(InferenceEngineImpl.BACKWARD_CHAINING);
		this.setOptionInference(InferenceEngineImpl.ASK_BASE);
	}
	...
	public Value<?> findValue(Variable variable)
		...
		try {
			...
			this.setOptionInference(InferenceEngineImpl.BASE);

			switch (this.getOptionInference()) {

			case InferenceEngineImpl.ASK_BASE:
				/* Pergunta ao usuario */
			...
			case InferenceEngineImpl.BASE_ASK:
				if (!this.proveVariable(variable)) {
					return null;
				}
				/* Pergunta ao usuario */
